...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart,delistItem}) => {
  ...
  return (
  <div key={product.id} className={`product-list-item ${product.stockQuantity===0 ? 'disabled' : '' }`}
    style={{ cursor: product.stockQuantity === 0 ? 'not-allowed' : 'pointer' }}>
    <img src={`/product-images/${product.name}.jpg`} alt={product.name} className="product-image" />
    <div className="product-details">
      <div className="product-card">
        <h3>{product.name}</h3>
        <p className="product-category">{product.category}</p>
        <p>${product.price.toFixed(2)}</p>
        {product.stockQuantity <= 0 ? ( <div className="sold-out">Sold Out
      </div>
      ) : product.stockQuantity <= 5 ? ( <button id="add-to-cart-button" onClick={handleAddToCart}
